...
// Loaded by vibe_agent_bridge.js inside one Rhino scope.
  ...
  function setupCodexFromMcpProject(options, codexHome, mcpEndpoint, profile) {
    ...
    var report = {
      attempted: false,
      ok: false,
      source: "lib_ConvertigoMCP._setupCodex",
      skillStatus: "",
      configStatus: "",
    ...
    try {
      var response = callLocalSequence("lib_ConvertigoMCP", "_setupCodex", {
      ...
      if (result === null) {
        throw new Error("lib_ConvertigoMCP._setupCodex did not return a setup result");
      }
      ...
      report.message = skillLabel + " skill synchronized from lib_ConvertigoMCP._setupCodex";
    } catch (e) {
      report.ok = false;
      report.error = String(e);
      report.message = "Unable to synchronize from lib_ConvertigoMCP._setupCodex; using bridge fallback skill generator";
    }
    return report;
  }
